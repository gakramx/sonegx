...
echo
	...
	#sonegxVersion='v21.12.02'

	isoLabel='sonegx'$(date +%Y.%m.%d)'-x86_64.iso'

	# setting of the general parameters
	archisoRequiredVersion="archiso 68-1"
	buildFolder=$HOME"/sonegx-build"
	outFolder=$HOME"/Sonegx-Out"
	archisoVersion=$(sudo pacman -Q archiso)

	echo "################################################################## "
	echo "Building the desktop                   : "$desktop
	#echo "Building version                       : "$sonegxVersion
...
echo "- Getting Sonegx key and mirror"
tput sgr0
echo "################################################################## "
echo
	...
	package="sonegx-keyring"

	#checking if application is already installed or else install
	if pacman -Qi $package &> /dev/null; then

			echo "################################################################"
			echo "Sonegx keyring is already installed"
			echo "################################################################"

	else

		wget https://github.com/gakramx/sonegx_repo/raw/main/x86_64/sonegx-keyring-1-1-any.pkg.tar.zst -O /tmp/sonegx-keyring-1-1-any.pkg.tar.zst
		sudo pacman -U --noconfirm --needed /tmp/sonegx-keyring-1-1-any.pkg.tar.zst
	...
	package="sonegx-mirrorlist"

	#checking if application is already installed or else install
	if pacman -Qi $package &> /dev/null; then

		echo "################################################################"
		echo "Sonegx mirrorlist is already installed"
		echo "################################################################"

	else

		wget https://github.com/gakramx/sonegx_repo/raw/main/x86_64/sonegx-mirrorlist-22.11-01-any.pkg.tar.zst -O /tmp/sonegx-mirrorlist-22.11-01-any.pkg.tar.zst
		sudo pacman -U --noconfirm --needed /tmp/sonegx-mirrorlist-22.11-01-any.pkg.tar.zst
	fi
...
echo
	...
	#profiledef.sh
	oldname1='iso_name="sonegx'
	newname1='iso_name="sonegx'

	oldname2='iso_label="sonegx'
	newname2='iso_label="sonegx'
